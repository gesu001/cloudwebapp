...
type Metrics = {
    ...
    recentRequests: { id: number; method: string; path: string; status: number; duration: number; createdAt: string; feed?: { name: string } }[];
};
...
export default function DashboardPage() {
    ...
    return (
        <main className="shell">
            ...
            <section className="panel activity"><div className="panel-heading"><div><p className="eyebrow">REQUEST LOG</p><h2>Recent activity</h2></div><span className="badge">{summary?.totalRequests ?? 0} tracked</span></div><div className="table-wrap"><table><thead><tr><th>Endpoint</th><th>Client</th><th>Status</th><th>Latency</th><th>Time</th></tr></thead><tbody>{metrics?.recentRequests.map((request) => <tr key={request.id}><td><code>{request.method} {request.path}</code></td><td>{request.feed?.name ?? "System"}</td><td><span className={`http-status ${request.status < 400 ? "ok" : "bad"}`}>{request.status}</span></td><td>{request.duration} ms</td><td>{new Date(request.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</td></tr>)}</tbody></table></div></section>
            {selectedFeed && <div className="modal-backdrop" onClick={() => setSelectedFeed(null)}><article className="reader" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setSelectedFeed(null)} aria-label="Close feed">x</button><p className="eyebrow">RSS READER</p><h2>{selectedFeed.name}</h2><p className="source">{selectedFeed.sourceUrl}</p>{selectedFeed.items?.map((item) => <div className="story" key={item.id}><p className="story-meta">{item.author} / {new Date(item.publishedAt).toLocaleDateString()}</p><h3>{item.title}</h3><p>{item.summary}</p><a href={item.link} target="_blank" rel="noreferrer">Read source &#8599;</a></div>)}</article></div>}
        </main>
    );
}

...
export default function FeedsPage() {
  ...
  return <main className="subpage shell"><p className="eyebrow">RSS CLIENT</p><h1>Feeds / Posts</h1><p className="subpage-lede">A learner-friendly reading view powered by the RSS Server API.</p>{error && <p className="alert" role="alert">{error}</p>}<div className="posts-grid">{feeds.flatMap((feed) => feed.items.map((item) => <article className="post-card" key={item.id}><p className="story-meta">{feed.name} / {new Date(item.publishedAt).toLocaleDateString()}</p><h2>{item.title}</h2><p>{item.summary}</p><p className="post-author">Posted by {item.author}</p><Link href={`/feeds/${feed.slug}`}>Read more &#8599;</Link></article>))}{!feeds.length && !error && <p className="empty">Loading posts from the RSS Server...</p>}</div></main>;
}
